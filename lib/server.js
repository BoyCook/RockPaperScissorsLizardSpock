...
//    game = process.env.RPSLP_COV ? require('../lib-cov/rpsls-app').newApp(db) : require('./rpsls-app').newApp(db);
    ...
    app.use(express.session({ secret: cookieSecret, key: cookieKey, store: new RedisStore({ client: db }), cookie: { maxAge: 60000, expires: false } }));
...
io.configure(function () {
    ...
    io.set('authorization', function (data, callback) {
        ...
        checkSocketSession(sessionID, callback);
    });
});

//TODO: move to authenticator and implement
function checkSocketSession(sessionID, callback) {
    //TODO: lookup
...
//    console.log('Socket SessionID [%s]', socket.handshake.sessionID);
