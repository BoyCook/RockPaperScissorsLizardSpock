...
//    TODO: use app.set('redisdb', 1); and db.select(app.set('redisdb'), function(err,res){});
});
...
app.get('/challenge/between/:user1/and/:user2', game.getChallengesBetweenUsers);
...
function checkSocketSession(data, callback) {
    ...
    console.log('Socket: User [%s] - SessionID [%s]', user, sessionID);
    sessionStore.get(sessionID, function(err, session) {
        if (err !== undefined) {
            console.log('Error getting session from SessionID [%s]', sessionID);
        } else {
